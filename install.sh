...
WRAPPER
    ...
    ok "Created: $LIPI_BIN_DIR/lipi (Interpreter & REPL)"

    # 2. Build native compiler if C compiler available
    local cc_cmd=""
    ...
    if [ -n "$cc_cmd" ] && [ -f "$LIPI_INSTALL_DIR/src/compiler/c_codegen.c" ]; then
        ...
        if $cc_cmd -O2 "$LIPI_INSTALL_DIR/src/compiler/c_codegen.c" \
            ...
            warn "Native binary compilation skipped (will use fallback)"
        fi
    fi
