...
install_lipi() {
    ...
    # Remove old install or pull latest
    if [ -d "$LIPI_INSTALL_DIR/.git" ]; then
...
create_command() {
    ...
    if [ ! -x "$LIPI_INSTALL_DIR/bin/lipc" ] && [ ! -x "$LIPI_INSTALL_DIR/bin/lipc_bin" ]; then
        local SEED_EXEC="/tmp/lipi_genesis_seed"
        if [ -f "$LIPI_INSTALL_DIR/src/boot/lipi-seed" ]; then
            cp "$LIPI_INSTALL_DIR/src/boot/lipi-seed" "$SEED_EXEC"
            chmod +x "$SEED_EXEC"
        elif [ -f "$LIPI_INSTALL_DIR/src/boot/seed.b64" ]; then
            base64 -d "$LIPI_INSTALL_DIR/src/boot/seed.b64" > "$SEED_EXEC"
            chmod +x "$SEED_EXEC"
        fi
        
        if [ -x "$SEED_EXEC" ]; then
            info "Bootstrapping Stage-3 Modular Silicon Compiler from genesis seed..."
            "$SEED_EXEC" "$LIPI_INSTALL_DIR/src/compiler/driver_cli.lp" "$LIPI_INSTALL_DIR/bin/lipc_bin" 2>/dev/null || true
            rm -f "$SEED_EXEC"
            if [ -f "$LIPI_INSTALL_DIR/bin/lipc_bin" ]; then
                chmod +x "$LIPI_INSTALL_DIR/bin/lipc_bin"
                ln -sf "$LIPI_INSTALL_DIR/bin/lipc_bin" "$LIPI_INSTALL_DIR/bin/lipc"
                ok "Stage-3 Compiler successfully instantiated from source"
            else
                # Fallback to direct seed copy if driver compilation failed
                if [ -f "$LIPI_INSTALL_DIR/src/boot/lipi-seed" ]; then
                    cp "$LIPI_INSTALL_DIR/src/boot/lipi-seed" "$LIPI_INSTALL_DIR/bin/lipc"
                else
                    base64 -d "$LIPI_INSTALL_DIR/src/boot/seed.b64" > "$LIPI_INSTALL_DIR/bin/lipc"
                fi
                chmod +x "$LIPI_INSTALL_DIR/bin/lipc"
                ln -sf "$LIPI_INSTALL_DIR/bin/lipc" "$LIPI_INSTALL_DIR/bin/lipc_bin"
            fi
        fi
    elif [ -x "$LIPI_INSTALL_DIR/bin/lipc" ] && [ ! -x "$LIPI_INSTALL_DIR/bin/lipc_bin" ]; then
        ln -sf "$LIPI_INSTALL_DIR/bin/lipc" "$LIPI_INSTALL_DIR/bin/lipc_bin"
    elif [ -x "$LIPI_INSTALL_DIR/bin/lipc_bin" ] && [ ! -x "$LIPI_INSTALL_DIR/bin/lipc" ]; then
        ln -sf "$LIPI_INSTALL_DIR/bin/lipc_bin" "$LIPI_INSTALL_DIR/bin/lipc"
    fi

    # Compile core toolchain binaries from source if this is a fresh source-only clone
    if [ -x "$LIPI_INSTALL_DIR/bin/lipc" ]; then
        info "Building sovereign developer toolchain from pure source..."
        [ -x "$LIPI_INSTALL_DIR/bin/lipi" ] || "$LIPI_INSTALL_DIR/bin/lipc" "$LIPI_INSTALL_DIR/src/tools/lipi.lp" "$LIPI_INSTALL_DIR/bin/lipi" 2>/dev/null && chmod +x "$LIPI_INSTALL_DIR/bin/lipi" 2>/dev/null || true
        [ -x "$LIPI_INSTALL_DIR/bin/lipipkg" ] || "$LIPI_INSTALL_DIR/bin/lipc" "$LIPI_INSTALL_DIR/src/tools/lipipkg.lp" "$LIPI_INSTALL_DIR/bin/lipipkg" 2>/dev/null && chmod +x "$LIPI_INSTALL_DIR/bin/lipipkg" 2>/dev/null || true
        [ -x "$LIPI_INSTALL_DIR/bin/lipilsp" ] || "$LIPI_INSTALL_DIR/bin/lipc" "$LIPI_INSTALL_DIR/src/tools/lipilsp.lp" "$LIPI_INSTALL_DIR/bin/lipilsp" 2>/dev/null && chmod +x "$LIPI_INSTALL_DIR/bin/lipilsp" 2>/dev/null || true
        [ -x "$LIPI_INSTALL_DIR/bin/lipidbg" ] || "$LIPI_INSTALL_DIR/bin/lipc" "$LIPI_INSTALL_DIR/src/tools/lipidbg.lp" "$LIPI_INSTALL_DIR/bin/lipidbg" 2>/dev/null && chmod +x "$LIPI_INSTALL_DIR/bin/lipidbg" 2>/dev/null || true
        [ -x "$LIPI_INSTALL_DIR/bin/lipifmt" ] || "$LIPI_INSTALL_DIR/bin/lipc" "$LIPI_INSTALL_DIR/src/tools/lipifmt.lp" "$LIPI_INSTALL_DIR/bin/lipifmt" 2>/dev/null && chmod +x "$LIPI_INSTALL_DIR/bin/lipifmt" 2>/dev/null || true
        [ -x "$LIPI_INSTALL_DIR/bin/lipiconvert" ] || "$LIPI_INSTALL_DIR/bin/lipc" "$LIPI_INSTALL_DIR/src/tools/lipiconvert.lp" "$LIPI_INSTALL_DIR/bin/lipiconvert" 2>/dev/null && chmod +x "$LIPI_INSTALL_DIR/bin/lipiconvert" 2>/dev/null || true
        [ -x "$LIPI_INSTALL_DIR/bin/lipidoc" ] || "$LIPI_INSTALL_DIR/bin/lipc" "$LIPI_INSTALL_DIR/src/tools/lipidoc.lp" "$LIPI_INSTALL_DIR/bin/lipidoc" 2>/dev/null && chmod +x "$LIPI_INSTALL_DIR/bin/lipidoc" 2>/dev/null || true
        [ -x "$LIPI_INSTALL_DIR/bin/lipirepl" ] || "$LIPI_INSTALL_DIR/bin/lipc" "$LIPI_INSTALL_DIR/src/tools/lipirepl.lp" "$LIPI_INSTALL_DIR/bin/lipirepl" 2>/dev/null && chmod +x "$LIPI_INSTALL_DIR/bin/lipirepl" 2>/dev/null || true
        [ -x "$LIPI_INSTALL_DIR/bin/lipi-build" ] || "$LIPI_INSTALL_DIR/bin/lipc" "$LIPI_INSTALL_DIR/build.lp" "$LIPI_INSTALL_DIR/bin/lipi-build" 2>/dev/null && chmod +x "$LIPI_INSTALL_DIR/bin/lipi-build" 2>/dev/null || true
        [ -x "$LIPI_INSTALL_DIR/bin/lipi-test" ] || "$LIPI_INSTALL_DIR/bin/lipc" "$LIPI_INSTALL_DIR/tests/run_tests.lp" "$LIPI_INSTALL_DIR/bin/lipi-test" 2>/dev/null && chmod +x "$LIPI_INSTALL_DIR/bin/lipi-test" 2>/dev/null || true
    fi
...
print_success() {
    ...
    echo ""
    echo "  Quick test:"
    echo -e "    ${LIPI_COLOR_YELLOW}lipi run examples/01_hello_world/main.lp${LIPI_COLOR_RESET}"
    echo ""
    echo "  Documentation:"
