...
PKG_WRAPPER
    ...
    # 5. Language Server Protocol (lipils) wrapper
    cat > "$LIPI_BIN_DIR/lipils" << LSP_WRAPPER
#!/bin/bash
# Lipi Programming Language — Language Server Protocol Wrapper
# Generated by install.sh — Do not edit manually
LIPI_HOME="$LIPI_INSTALL_DIR"
exec "$LIPI_INSTALL_DIR/bin/lipils" "\$@"
LSP_WRAPPER
    chmod +x "$LIPI_BIN_DIR/lipils"
    ok "Created: $LIPI_BIN_DIR/lipils (Language Server Protocol Engine)"
}
