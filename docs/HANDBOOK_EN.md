...
- **x86_64 ELF64 Emitter:** [`src/compiler/elf_emitter.lp`](../src/compiler/elf_emitter.lp)
- **ARM64 (AArch64) ELF64 Emitter:** [`src/compiler/arm64_emitter.lp`](../src/compiler/arm64_emitter.lp)
- **WebAssembly (WASM) Binary Emitter:** [`src/compiler/wasm_emitter.lp`](../src/compiler/wasm_emitter.lp)
...
Located at [`src/tools/lipidbg.lp`](../src/tools/lipidbg.lp), `lipidbg` inspects compiled ELF binaries and runtime environments without GDB:
...
Located at [`src/tools/lipirepl.lp`](../src/tools/lipirepl.lp), `lipirepl` offers immediate feedback and low-level introspection:
...
Located at [`src/tools/lipipkg.lp`](../src/tools/lipipkg.lp), `lipipkg` orchestrates the complete software lifecycle:
...
Projects are configured using the standard [`lipipkg.toml`](./LIPIPKG_TOML_SPEC.md) manifest:
...
The official LiPi editor tooling and VS Code extension are maintained in a dedicated standalone repository:
- **Repository:** [`vscode-lipi`](https://github.com/asaudola-cmyk/vscode-lipi)
...
- Code snippets and full language templates (`snippets/lipi.json`).
- First-class Language Server Protocol daemon (`bin/lipilsp`).

### Installation
```bash
# Clone and link extension to VS Code extensions directory:
git clone https://github.com/asaudola-cmyk/vscode-lipi.git ~/.vscode/extensions/lipi-lang-1.0.0
```
